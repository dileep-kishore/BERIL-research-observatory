...
*Confirmed NOT FOUND via Spark SQL (2026-01-07)*

The following tables were mentioned in project documentation but do not exist in the current database:

| Table Name | Mentioned For | Status |
|------------|---------------|--------|
| `phylogenetic_tree` | Species phylogenies from single-copy core genes | **NOT FOUND** |
| `phylogentic_tree_distance_pairs` | Pairwise phylogenetic distances | **NOT FOUND** |
...
These tables may be planned for future releases or stored in a different location. The `protocol_id` column in `pangenome` and `genome_ani` tables references `pangenome_build_protocol`, but the protocol table itself is not available.
